...
df_titanic<- read.csv('https://web.stanford.edu/class/archive/cs/cs109/cs109.1166/stuff/titanic.csv') %>%
  mutate(Fare=round(Fare),
         independent=log(Fare),#Fare to opłata za bilet, tak przekształcona zmienna lepiej uwydatni transformacje na zaleznosc liniowa
         independent=ifelse(independent==-Inf,0,independent)) 

```

```{r echo=FALSE}
df_titanic %>% select(Survived,independent) %>% head() %>% kable()
# Wyliczanie modelu logistycznego.
model <- glm(data=df_titanic, Survived ~ independent, family = 'binomial')
...
# wyliczam confidence interval
confint(model)

paste0('CI of coefficient between ',
       round(broom::tidy(model)$estimate[2] - broom::tidy(model)$std.error[2]*1.95,3),
       ' and ',
       round(broom::tidy(model)$estimate[2] + broom::tidy(model)$std.error[2]*1.95,3)
)
```
...
p <-predict(model,data.frame(list('independent'=c(1,2))), type = "response",se.fit=FALSE)
p1 = p[1]
p2 = p[2]
...
Prob for independent= 1 was equal to `r p1` while for independent = 2 was equal to `r p2`. The odds increase by `r x`. The same what model results suggests -\> `r x1`.
...
# ten wykres najsilniej daje pokaz na zmiennej zlogarytmowany Fare = log(Fare)
...
df_aug %>% 
  ggplot()+
  geom_line(aes(x=independent,y=.fitted), color='green')+
  geom_line(aes(x=independent,y=odds ), color='red')+
  geom_line(aes(x=independent,y=log_odds ), color='blue', label='log_odds')
```
```{r evaluating_model_quality}
